package org.example.bigdisplayproject.feature.display.presentation.util

object Constants {

    // dimensions in Pixels
    const val BACK_BUTTON_HEIGHT = 40
    const val BACK_BUTTON_WIDTH = 120

    const val BOTTOM_PANEL_HEIGHT = 100
    const val BOTTOM_PANEL_ICONS_SPACE = 30

    const val PADDING_BETWEEN_CARDS = 30

    const val CARD_DETAIL_WIDTH = 1700
    const val CARD_DETAIL_HEIGHT = 700
    const val CARD_DETAIL_PADDING = 50
    const val CARD_DETAIL_BETWEEN = 64
    const val CARD_DETAIL_PADDING_TEXT = 30
    const val CLOSE_BUTTON_SIZE = 60
    const val CLOSE_BUTTON_PADDING = 20

    const val CARD_WIDTH = 300
    const val CARD_TEXT_HEIGHT = 80
    const val NO_IMAGE_CARD_HEIGHT = 150
    const val CUSTOM_IMAGE_HEIGHT = 1024 // заглушка
    const val CUSTOM_IMAGE_WIDTH = 1440 // заглушка
}